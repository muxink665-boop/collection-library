...
const categories={books:{label:'书籍',icon:'书',className:'book',description:'收集纸张里的宇宙，以及阅读曾抵达过的地方。',fields:'书名、作者、书籍类型、收藏介质、出版社、ISBN、版本、出版年份、购入渠道、阅读状态、备注',genres:['科幻','文学小说','推理悬疑','历史','传记','艺术设计','人文社科','漫画绘本','诗歌','其他']},music:{label:'音乐',icon:'音',className:'music',description:'保存声音，也保存当时听到它的那个瞬间。',fields:'专辑名、艺人、收藏介质（CD / 黑胶）、音乐类型、厂牌、发行年份、版本、碟片编号、购入渠道、保存状态、备注',genres:['摇滚','流行','爵士','古典','电子','民谣','嘻哈','原声带','世界音乐','其他']},movies:{label:'电影',icon:'影',className:'movie',description:'把银幕记忆延伸到日常生活里。',fields:'电影名、导演、电影类型、收藏介质、国家 / 地区、上映年份、版本、尺寸或规格、购入渠道、保存状态、备注',genres:['剧情','爱情','科幻','悬疑','恐怖','喜剧','动画','纪录片','犯罪','动作']}};
  ...
  supabase.auth.onAuthStateChange((event,session)=>{
    ...
    if(currentUser){
      ...
      (async()=>{try{await supabase.rpc('claim_orphans');}catch(e){}
        const {entries:e,trash:t,migrated}=await loadData();entries=e;trash=t;if(migrated)await persist();renderCurrent();})();
...
(async()=>{await initAuth();const {entries:e,trash:t,migrated}=await loadData();entries=e;trash=t;if(migrated)await persist();renderCurrent();})();
